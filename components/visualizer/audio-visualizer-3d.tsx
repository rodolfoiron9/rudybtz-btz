...
function VisualizerMesh({ audioDataRef, preset }: VisualizerMeshProps) {
  ...
  // Helper objects for calculations - created once to avoid GC
  ...
  // Initial positioning of instances
  useEffect(() => {
  ...
  useFrame((state) => {
    ...
    for (let x = 0; x < gridSize; x++) {
      for (let z = 0; z < gridSize; z++) {
        ...
        const frequencyIndex = Math.floor((index / instanceCount) * frequencies.length);
    ...
    // Update lights directly via refs
