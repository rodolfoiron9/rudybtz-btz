...
interface VisualizerMeshProps {
  audioDataRef: RefObject<AudioData | null>;
...
function VisualizerMesh({ audioDataRef, preset }: VisualizerMeshProps) {
  ...
  useFrame((state) => {
    ...
    if (!audioData || !instancedMesh) return;
    ...
    for (let x = 0; x < gridSize; x++) {
      for (let z = 0; z < gridSize; z++) {
        const index = i++;
        const frequencyIndex = Math.floor((index / (gridSize * gridSize)) * frequencies.length);
        ...
        color.setHSL(hue, 0.8, 0.5 + normalizedFreq * 0.2);
    ...
    if (preset.effects.rotation) {
      instancedMesh.rotation.y = time * 0.5 + average * 0.01;
    }
