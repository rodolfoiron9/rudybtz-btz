...
// Pre-allocate temporary objects to avoid garbage collection in useFrame
...
function VisualizerMesh({ audioDataRef, preset }: VisualizerMeshProps) {
  ...
  useEffect(() => {
    if (!instancedMeshRef.current || !instancedMeshRef.current.setMatrixAt) return;
    ...
    for (let x = 0; x < gridSize; x++) {
      for (let z = 0; z < gridSize; z++) {
        _object.position.set(x * spacing - offset, 0, z * spacing - offset);
        _object.updateMatrix();
        instancedMeshRef.current.setMatrixAt(i++, _object.matrix);
      }
    }
  ...
  useFrame((state) => {
    ...
    if (!audioData || !instancedMeshRef.current || !instancedMeshRef.current.setMatrixAt) return;
    ...
    for (let x = 0; x < gridSize; x++) {
      for (let z = 0; z < gridSize; z++) {
        ...
        if (preset.effects.scaling) {
          ...
          _object.scale.setY(scaleY);
        }

        // Apply rotation based on audio
        if (preset.effects.rotation) {
          _object.rotation.y = time + normalizedFreq * Math.PI;
        }

        // Apply pulsing effect
        if (preset.effects.pulsing) {
          const pulse = Math.sin(time * 4 + index * 0.1) * 0.2 + 1;
          _object.scale.setX(pulse);
          _object.scale.setZ(pulse);
        }

        // Position modulation for wave effect
        if (preset.type === 'waves') {
          _object.position.y = Math.sin(time * 2 + index * 0.2) * normalizedFreq * 2;
        }

        _object.updateMatrix();
        instancedMeshRef.current.setMatrixAt(index, _object.matrix);
        ...
        if (instancedMeshRef.current.setColorAt) {
          ...
          instancedMeshRef.current.setColorAt(index, _color);
        }
      }
    }
    ...
    // Group rotation (applied to the mesh itself as we don't have a group wrapper for instances anymore)
    if (preset.effects.rotation) {
  ...
  return (
    <>
      <instancedMesh ref={instancedMeshRef} args={[geometry, material, count]}>
        {/* No children needed, instances are managed via API */}
      </instancedMesh>
...
export default function AudioVisualizer3D({ 
  ...
  const initializeAudio = useCallback(async () => {
    ...
    try {
      ...
      // Pre-allocate audio data buffers
  ...
  const analyzeAudio = useCallback(() => {
    ...
    if (!analyserRef.current || !currentData) return;

    const analyser = analyserRef.current;
    const { frequencies, waveform } = currentData;

    // Use any cast to satisfy type checker for getByteFrequencyData with Uint8Array
    (analyser as any).getByteFrequencyData(frequencies);
    (analyser as any).getByteTimeDomainData(waveform);
    ...
    const bassEnd = Math.floor(frequencyCount * 0.1);
    const midEnd = Math.floor(frequencyCount * 0.3);
    
    let bassSum = 0;
    for (let i = 0; i < bassEnd; i++) {
      const val = frequencies[i];
      if (val !== undefined) bassSum += val;
    }
    ...
    for (let i = bassEnd; i < midEnd; i++) {
      const val = frequencies[i];
      if (val !== undefined) midSum += val;
    }
    const mid = midSum / (midEnd - bassEnd || 1);

    let trebleSum = 0;
    for (let i = midEnd; i < frequencyCount; i++) {
      const val = frequencies[i];
      if (val !== undefined) trebleSum += val;
    }
    const treble = trebleSum / (frequencyCount - midEnd || 1);

    // Update ref directly to bypass React re-renders
    currentData.average = average;
    currentData.bass = bass;
    currentData.mid = mid;
    currentData.treble = treble;
