...
const MobileNavLink = ({ href, children, onOpenChange }: { href: string; children: React.ReactNode, onOpenChange: (open: boolean) => void }) => (
    <Link href={href} className="flex items-center gap-4 px-4 py-3 text-lg font-semibold transition-colors rounded-lg text-primary-foreground/80 hover:bg-white/10 hover:text-primary-foreground" onClick={() => onOpenChange(false)}>
    {children}
  </Link>
)
